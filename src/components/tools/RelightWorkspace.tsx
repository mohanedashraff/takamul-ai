      ...
      <div className="flex flex-col flex-1 overflow-hidden pt-16 md:pt-20">
        ...
        {(phase === "edit" || phase === "processing") && (
          <div className="shrink-0 flex items-center gap-3 px-4 py-3 border-t border-white/5 bg-black/50 backdrop-blur-sm">
            ...
            {/* Globe compact */}
            <div className="shrink-0 rounded-xl bg-[#111] border border-white/8 p-1.5">
              <div className="relative flex items-center justify-center" style={{ width: 80, height: 80 }}>
                <button onClick={() => nudge(0, 10)}
                  className="absolute top-0 left-1/2 -translate-x-1/2 w-5 h-5 flex items-center justify-center text-gray-500 hover:text-white transition-colors z-10">
                  <ChevronUp className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => nudge(0, -10)}
                  className="absolute bottom-0 left-1/2 -translate-x-1/2 w-5 h-5 flex items-center justify-center text-gray-500 hover:text-white transition-colors z-10">
                  <ChevronDown className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => nudge(-10, 0)}
                  className="absolute left-0 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center text-gray-500 hover:text-white transition-colors z-10">
                  <ChevronLeft className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => nudge(10, 0)}
                  className="absolute right-0 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center text-gray-500 hover:text-white transition-colors z-10">
            ...
            <div className="shrink-0 flex flex-wrap gap-1 max-w-[140px]">
              {QUICK.map(q => (
                <button key={q.label}
                  ...
                  className="h-7 px-2 rounded-lg text-xs font-medium bg-white/6 border border-white/10 text-gray-300 hover:bg-white/12 hover:text-white hover:border-white/20 transition-all">
                  {q.label}
                </button>
              ))}
            </div>
            ...
            {/* Light type */}
            <div className="shrink-0 flex rounded-xl overflow-hidden border border-white/10">
              {(["soft", "hard"] as const).map(t => (
                <button key={t} onClick={() => setLightType(t)}
                  className={cn("px-3 h-8 text-xs font-semibold transition-all",
            ...
            <div className="shrink-0 flex items-center gap-2 min-w-[100px]">
              <span className="text-xs text-gray-500 whitespace-nowrap">السطوع</span>
              <input type="range" min={0} max={100} value={brightness}
                onChange={e => setBrightness(Number(e.target.value))}
                className="w-20 h-1.5 rounded-full"
                style={{ accentColor: `rgb(${rgb})` }} />
              <span className="text-xs font-mono font-bold w-8" style={{ color: `rgb(${rgb})` }}>{brightness}%</span>
            </div>
            ...
            <div className="shrink-0 flex items-center gap-1.5">
              ...
              <label className="flex items-center gap-1.5 cursor-pointer">
                ...
                <span className="text-[10px] font-mono text-gray-400 uppercase">{lightColor}</span>
              </label>
            </div>
            ...
            <div className="shrink-0 flex flex-col items-end gap-1.5">
              ...
              >
                {phase === "processing"
                  ...
                  : <><Sparkles className="w-4 h-4" /> تطبيق الإضاءة</>}
              </button>
            </div>
