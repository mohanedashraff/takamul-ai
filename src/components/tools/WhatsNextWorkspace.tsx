...
import {
  ...
  Loader2, Download, RefreshCw, Check,
} from "lucide-react";
...
const SHOT_STYLES: ShotStyle[] = [
  { id: "auto",     label: "تلقائي",       labelEn: "Auto",        icon: "✨", desc: "الذكاء الاصطناعي يختار" },
  { id: "wide",     label: "لقطة واسعة",   labelEn: "Wide Shot",   icon: "🌄", desc: "منظور شامل للمشهد" },
  { id: "closeup",  label: "لقطة مقربة",   labelEn: "Close-Up",    icon: "🎯", desc: "تفاصيل وجه أو عنصر" },
  { id: "medium",   label: "لقطة متوسطة",  labelEn: "Medium",      icon: "🎬", desc: "الجزء العلوي من الجسم" },
  { id: "aerial",   label: "جوية",         labelEn: "Aerial",      icon: "🛸", desc: "من الأعلى للأسفل" },
  { id: "pov",      label: "منظور الشخص",  labelEn: "POV",         icon: "👁", desc: "ترى ما يراه الشخص" },
  { id: "dramatic", label: "درامي",        labelEn: "Dramatic",    icon: "🎭", desc: "زوايا وإضاءة مكثفة" },
  { id: "action",   label: "حركة",         labelEn: "Action",      icon: "⚡", desc: "مشهد ديناميكي سريع" },
];

// ── Grid count options ─────────────────────────────────────────────────────────

const GRID_OPTIONS = [
  { n: 2,  label: "2",  cols: "grid-cols-2", desc: "٢ نتيجة" },
  { n: 4,  label: "4",  cols: "grid-cols-2", desc: "٤ نتائج" },
  { n: 6,  label: "6",  cols: "grid-cols-3", desc: "٦ نتائج" },
  { n: 8,  label: "8",  cols: "grid-cols-4", desc: "٨ نتائج" },
];
...
export function WhatsNextWorkspace({ tool, config }: Props) {
  ...
  const [shotStyle,   setShotStyle]   = useState("auto");
  ...
  const reset = () => { clearFile(); setPhase("idle"); setShotStyle("auto"); setGridCount(4); };

  const canGenerate = !!preview;
  const activeStyle = SHOT_STYLES.find(s => s.id === shotStyle)!;
  const activeGrid  = GRID_OPTIONS.find(g => g.n === gridCount)!;

  // Fake result images (same preview with slight filter variations)
  const resultImages = Array.from({ length: gridCount }, (_, i) => ({
    key: i,
    filter: [
      "none",
                ...
                >
                  ...
                  <div className="flex flex-col gap-2.5">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-semibold text-gray-300">نوع اللقطة</p>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {SHOT_STYLES.map(style => {
                        const active = shotStyle === style.id;
                        return (
                          <button
                            key={style.id}
                            onClick={() => setShotStyle(style.id)}
                            className={cn(
                              "relative flex flex-col items-center gap-1.5 px-2 py-3 rounded-2xl border transition-all duration-200",
                              "hover:scale-[1.03]",
                              active ? "text-white" : "text-gray-500 hover:text-gray-300 bg-white/3 border-white/8"
                            )}
                            style={active ? {
                          ...
                          >
                            ...
                            <span className="text-[11px] font-bold leading-tight text-center">{style.label}</span>
                            <span className="text-[9px] text-gray-600 leading-tight text-center">{style.desc}</span>
                            {active && (
                              <div className="absolute top-1.5 left-1.5 w-3.5 h-3.5 rounded-full flex items-center justify-center"
                  ...
                  {/* ── Grid count selector ── */}
                  <div className="flex flex-col gap-2.5">
                    <p className="text-sm font-semibold text-gray-300">عدد النتائج</p>
                    <div className="flex gap-2">
                      {GRID_OPTIONS.map(opt => {
                        const active = gridCount === opt.n;
                        return (
                          <button
                            key={opt.n}
                            onClick={() => setGridCount(opt.n)}
                            className={cn(
                              "flex-1 h-12 rounded-2xl border font-bold text-lg transition-all duration-200",
                              "hover:scale-[1.04]",
                              active ? "text-white" : "bg-white/3 border-white/8 text-gray-500 hover:text-gray-300"
                            )}
                            style={active ? {
                    ...
                    <p className="text-xs text-gray-600 text-center">
                      سيتم توليد <span style={{ color: `rgb(${rgb})` }}>{gridCount}</span> احتمالات للمشهد التالي
                    </p>
                  </div>
                  ...
                  >
                    ...
                    {canGenerate
                      ? `توليد ${gridCount} مشاهد — ${activeStyle.label}`
                      : "ارفع صورة للبدء"}
                  </button>
                </motion.div>
              )}
                    ...
                    <div className="relative" style={{ height: 220 }}>
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-gray-400 text-sm">{gridCount} احتمالات · {activeStyle.label}</p>
                        </div>
                      </div>
                    </div>

                    {/* Animated result grid preview (skeletons) */}
                    <div className={cn("grid gap-px bg-white/5", activeGrid.cols)}>
                    ...
                    <div className={cn("grid gap-px bg-white/5", activeGrid.cols)}>
                      {resultImages.map(({ key, filter }) => (
                        <motion.div key={key}
                          ...
                          style={{ height: gridCount <= 4 ? 160 : 110 }}
                        >
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          ...
                          <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/70 to-transparent py-2 px-2">
                            <p className="text-white text-[10px] font-bold">
                              {activeStyle.icon} {activeStyle.label} {key + 1}
                            </p>
                          </div>
                        </motion.div>
                      ))}
