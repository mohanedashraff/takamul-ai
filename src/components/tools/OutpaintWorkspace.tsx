  ...
  return (
    <div className="flex flex-col min-h-screen bg-bg-primary">
      ...
      <div className="flex flex-col flex-1 overflow-hidden pt-16 md:pt-20">
        ...
        {/* ── CANVAS AREA ──────────────────────────────────────────────── */}
        <div ref={containerRef} className="flex-1 flex items-center justify-center overflow-hidden bg-[#060608]">
        ...
        {(phase === "edit" || phase === "processing") && (
          <div className="shrink-0 flex items-center gap-3 px-5 py-3 border-t border-white/5 bg-black/40 backdrop-blur-sm">
            ...
            <div className="shrink-0 flex flex-wrap gap-1 max-w-[200px]">
              {PRESETS.map((p, i) => (
                <button
                  ...
                  disabled={!nat}
                  className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium bg-white/5 border border-white/8 text-gray-400 hover:text-white hover:bg-white/10 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  <span className="text-sm leading-none opacity-70">{p.icon}</span>
                  {p.label}
                </button>
              ))}
            </div>
            ...
            {/* Dimensions display */}
            {nat && hasExpansion && (
              <div className="shrink-0 text-xs font-mono text-center space-y-0.5">
                <div className="text-gray-600">{nat.w}×{nat.h}</div>
                <div className="font-bold" style={{ color: `rgb(${rgb})` }}>{outputW}×{outputH}</div>
              </div>
            )}
            ...
            <div className="shrink-0 flex flex-col items-end gap-1.5">
              ...
              >
                {phase === "processing"
                  ? <><Loader2 className="w-4 h-4 animate-spin" /> جاري التمديد...</>
                  : <><Sparkles className="w-4 h-4" /> ابدأ التمديد</>}
              </button>
            </div>
          </div>
        )}

        {/* Hint when no expansion yet */}
        {phase === "edit" && !hasExpansion && (
          <div
            className="shrink-0 py-2 text-xs text-center"
            style={{ color: "rgba(255,255,255,0.3)" }}
          >
            <Move className="w-3.5 h-3.5 inline ml-1.5 opacity-50" style={{ color: `rgb(${rgb})` }} />
